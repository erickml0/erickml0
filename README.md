<div align="center">

# Hola, soy Erick!

### Desarrolador web

[Portafolio](https://erickml0.github.io)

</div>

---

## Sobre mí

Me especializo en el desarrollo de aplicaciones web, con un enfoque principal en el frontend. Disfruto trabajar en la creación de interfaces, así como en la integración con APIs, bases de datos y los distintos componentes que conforman una aplicación web.

## Experiencia

Desarrollo de plataformas digitales para instituciones culturales:
- 📚 Repositorios digitales con Tainacan
- 🏛️ Aplicaciones web con React + Node
- 🔄 Pipelines ETL con Python + Airflow
- 🎨 Interfaces accesibles y optimizadas

## Mi Stack

![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/-React-61DAFB?style=flat&logo=react&logoColor=black)
![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat&logo=node.js&logoColor=white)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat&logo=python&logoColor=white)
![PHP](https://img.shields.io/badge/-PHP-777BB4?style=flat&logo=php&logoColor=white)
![Apache Airflow](https://img.shields.io/badge/-Apache_Airflow-017CEE?style=flat&logo=Apache%20Airflow&logoColor=white)
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Linux](https://img.shields.io/badge/-Linux-FCC624?style=flat&logo=linux&logoColor=black)
![Git](https://img.shields.io/badge/-Git-F05032?style=flat&logo=git&logoColor=white)

---
